...
Linnet 是一条模块化流水线，专门用来自动化研究者和工程师的日常信息采集习惯。它会从你关心的来源里筛选信号，用 LLM 提炼重点，再把整理好的 briefing 发送到你选择的 sinks。
...
`docs/data/` 下生成的 JSON 在普通本地工作区里默认会被忽略，避免产生噪音 diff；但定时 digest workflows 会强制加入并提交这些发布快照，这样你的日、周、月归档才能持续保留在 GitHub Pages 上。
...
在 GitHub Actions 构建时，Linnet 会根据仓库名自动推断正确的 GitHub Pages base path，所以 `/setup/`、`/daily/` 等内部链接会指向你自己的项目站点，而不是上游 `Linnet` 演示站。如果你使用自定义域名或更复杂的部署方式，可以通过 `SITE_URL`，以及在需要时配合 `SITE_BASE` 来覆盖。

### 4. 打开设置向导

当你自己的 GitHub Pages 站点部署好之后，直接打开你站点下的 `/setup/zh/` 就是最短路径。如果你只是想先看看上游演示，可以访问 [上游中文设置演示](https://yuyangxueed.github.io/Linnet/setup/zh/)。
...
对于 Step 6 的 `Connect GitHub` 一键部署路径，你需要一个具备正确权限的 GitHub Personal Access Token：
...
| Administration | 如果你想让 Step 6 自动启用 GitHub Actions 和 workflows，则需要 Read and write |
| Contents | Read and write |
| Metadata | Read-only（自动勾选） |
| Secrets | Read and write |

将 **Repository access** 设为 **Only select repositories**，然后只勾选你的 fork，不要选择 "All repositories"。

**Classic PAT** — 勾选 `repo`（全部子项）和 `workflow`。

如果你想让设置向导在 Step 6 里替你重新启用仓库级 GitHub Actions，并重新启用 `daily.yml`、`weekly.yml`、`monthly.yml`、`pages.yml` 这些 workflow，就打开 `自动启用 GitHub Actions 和 workflows` 开关。若不打开，就在 `Actions` 标签页里手动启用。

设置向导里的 **Instructions** 链接会逐项解释每个字段。
