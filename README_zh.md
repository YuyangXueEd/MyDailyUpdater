...
使用这个模板创建自己的仓库，填入一个 API key，用 GitHub Actions 跑起来。不要服务器，不要订阅，也不会把内容锁死在某个平台里。
...
到了 Step 6，确认目标仓库后，点击 **部署到 GitHub**。

现在默认的一键部署路径走的是 Linnet Bridge GitHub App，不再是 PAT。成功后它会：
...
大多数用户在 Step 6 之后不需要再手动点任何按钮。你只要在仓库里观察这两个 workflow：
