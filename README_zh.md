...
点击 GitHub 页面上的 **Use this template → Create a new repository**。这是推荐路径——新建仓库默认启用 GitHub Actions。

> **使用 Fork？** Fork 后的仓库默认禁用 Actions。Fork 完成后请先进入仓库的 **Actions** 标签页，点击 **"I understand my workflows, go ahead and enable them"**，否则所有 workflow 都不会运行。

### 2. 添加 API 密钥

如果你走手动配置路径，请进入你 fork 后仓库的 **Settings → Secrets and variables → Actions → New repository secret**。

| 名称 | 值 |
|---|---|
| `OPENROUTER_API_KEY` | 你在 [openrouter.ai/keys](https://openrouter.ai/keys) 获取的密钥 |

如果你在 Step 6 里使用 wizard 的 GitHub 一键部署，也可以直接把这个 key 填在那里，让浏览器帮你写入 secret。
...
推荐直接用 [Setup Wizard](https://yuyangxueed.github.io/Linnet/setup/zh/)。它会帮你完成来源选择、顺序调整、sinks 选择，以及生成适用于**你自己 fork** 的配置文件。Step 6 里你可以继续手动复制，也可以直接用 GitHub 一键部署，把文件和 secrets 写进你自己的仓库。
...
像 weather 的城市、arXiv 的过滤规则、GitHub Trending 的数量限制这类参数，都放在 `config/extensions/<name>.yaml` 里。
...
- Astro setup wizard 使用的 extension metadata 现在放在 `extensions/<name>/meta.json`
- 每个 extension 自己的细节配置，写在各自目录下的 `README.md`
- 新扩展可以直接复制 [`extensions/_template/`](extensions/_template/)

如果你想启用浏览器里的 `Connect GitHub` 一键部署路径，还需要先为 Astro 站点配置 `PUBLIC_GITHUB_APP_CLIENT_ID` 和 `PUBLIC_GITHUB_APP_CLIENT_SECRET`。因为这两个值会暴露给前端，这条路径现在明确标成 experimental，并在 UI 里附带泄露风险说明链接。
